...
| `BirthOrAdoptAsToddler` | bool | If true, the kid arrives as a toddler, skipping the newborn/baby/crawler phases. This is always true for `AdoptedFromNPC` kids but useful for case where the adopted entity doesn't have a real associated NPC. |
...
| `KidDialogueSheetName` | string | _null_ | If set, this will be the dialogue asset name used for the Child. The final asset name will be `Characters/Dialogue/<KidDialogueSheetName OR KidId>`. |
| `NPCDialogueSheetName` | string | _null_ | If set, this will be the dialogue asset name used for the kid NPC. The final asset name will be `Characters/Dialogue/<NPCDialogueSheetName OR KidId>`. For `AdoptedFromNPC` kids, this asset does not take effect until after you've adopted them. |
...
```json
{
  ...
  "Entries": {
    "{{ModId}}_CustomKid1": {
      ...
      // Optional fields
      "BirthOrAdoptAsToddler": <true if kid should arrive as a toddler>,
