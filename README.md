...
- `LittleNPC`: HMK implements a similar feature that allows content packs to add custom kids who grow up into NPC. The HMK side feature is automatically disabled if LittleNPC is installed and applicable to a particular kid. Thus it should be possible to use HMK for the general tweaks, and LittleNPC for the child to NPC aspect.
